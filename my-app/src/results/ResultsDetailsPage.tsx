...
export default function ResultDetailPage() {
  ...
  const initial =
    location.state?.row ?? RESULTS_DATA.find((r) => r.id === id) ?? null;
  ...
  const toggleFlag = () =>
    setRow((r) => (r ? { ...r, flagged: !r.flagged } : r));
  const toggleAccept = () =>
  ...
  return (
    <div className="w-full space-y-6">
      ...
      <section ref={ringRef} className="flex items-center gap-8">
        ...
        <div>
          <div
            className={`text-3xl font-extrabold ${verdictColor(row.verdict)}`}
          >
            {row.verdict.toUpperCase()}
          </div>
          <div className="text-zinc-500 mt-1">
            Amount: <span className="font-mono">{MYR.format(row.amount)}</span>
          </div>
        </div>
      </section>
